...
### Study of MSE and Prior sensitivity

We first simulate $n = 500$ observations from the following over-dispersed Poisson model.
...
Try a bunch of models, with different priors in terms of `u`:
...
Write a function to get such plot for each `alpha_vec` and `u_vec`:
...
Eventually different `alpha` can still be written into different choices of `u` in the prior since the exponential distribution is characterized by a single parameter.
...
Compute the MSE and coverage rate for different models:
...
Finally, let's put the sensitivity by median plots for the two settings in the same figure, with different colors:
