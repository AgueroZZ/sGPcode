...
xHeart = x[grepl("heart", x$Cause) & x$province == "Ontario", ]
xPreCovid = xHeart[xHeart$date < as.Date("2020/03/01"),]
xPostCovid = xHeart[xHeart$date >= as.Date("2020/03/01"),]

# xPreCovid = xHeart # not worry about prediction for now
...
B1_full_exact <- rbind(0,Compute_design_Aug_sGP(t[-1])[1:(nrow(xHeart)-1), ])
X_full <- as(cbind(cos(a1*t), sin(a1*t), 1, t), "dgTMatrix")
B_full <- cbind(B1_full_exact)

xPostCovid = xHeart
samps_g_pred <- as.matrix(B_full %*% samps_w) + as.matrix(X_full %*% samps_beta)
...
### Interval:
samps_g_pred$mean <- apply((samps_g_pred)[,1:3000],1, mean)
samps_g_pred$upper <- apply((samps_g_pred)[,1:3000],1, quantile, 0.975)
samps_g_pred$lower <- apply((samps_g_pred)[,1:3000],1, quantile, 0.025)
plot(mean~t, data = samps_g_pred, type = 'l', ylim = c(5,7))
lines(upper~t, data = samps_g_pred, lty = 'dashed')
lines(lower~t, data = samps_g_pred, lty = 'dashed')
points(log(xHeart$VALUE)~I(sort(t) + 2010))
...
plot(mean~t, data = samps_g_pred, type = 'l', ylim = c(280,600))
lines(upper~t, data = samps_g_pred, lty = 'dashed')
lines(lower~t, data = samps_g_pred, lty = 'dashed')
points((xHeart$VALUE)~I(sort(t) + 2010))
...
pdf(file = paste0(figure_path, "heart_log.pdf"), height = 5, width = 5)
plot((xHeart$VALUE)~I(sort(t) + 2010), cex = 0.5, lwd = 0.3,
     type = 'p', log = "y", 
     col = rgb(0,0,0,alpha = 0.3), xlab = "year", ylab = "Heart Attack Death", ylim = c(280, 600))
...
pdf(file = paste0(figure_path, "heart.pdf"), height = 5, width = 5)
plot((xHeart$VALUE)~I(sort(t) + 2010), cex = 0.5, lwd = 0.3,
     type = 'p', ylim = c(280, 600), col = rgb(0,0,0,alpha = 0.3),
     xlab = "year", ylab = "Heart Attack Death")
lines(mean~t, data = samps_g_pred, col = "blue", lwd = 1.3)
lines(upper~t, data = samps_g_pred, lty = 'dashed', col = "red", lwd = 1.3)
lines(lower~t, data = samps_g_pred, lty = 'dashed', col = "red", lwd = 1.3)
...
sample_excess_log <- log(xHeart$VALUE[-(1:nrow(xPreCovid))]) - log(samps_g_pred_all_obs[,1:3000])
sample_excess_log_total <- apply(na.omit(sample_excess_log), 2, sum)
pdf(file = paste0(figure_path, "heart_excess_log.pdf"), height = 5, width = 5)
hist((sample_excess_log_total), xlab = "Heart Attack Total Excess Mortality (Log)", main = "Posterior Samples", breaks = 30)
dev.off()
sample_excess <- (xHeart$VALUE[-(1:nrow(xPreCovid))]) - (samps_g_pred_all_obs[,1:3000])
sample_excess_total <- apply(na.omit(sample_excess), 2, sum)
pdf(file = paste0(figure_path, "heart_excess.pdf"), height = 5, width = 5)
hist((sample_excess_total), xlab = "Heart Attack Total Excess Mortality", main = "Posterior Samples", breaks = 30)
dev.off()


sample_excess_log <- cbind(t = (samps_g_pred_all$t), sample_excess_log)
pdf(file = paste0(figure_path, "heart_log_excess_path.pdf"), height = 5, width = 5)
matplot(y = (sample_excess_log[,2:31]), x = sample_excess_log[,1], type = 'l',
        add = F, lty = 'dashed', col = 'pink', ylab = "Heart Attack Excess Mortality (Log)",
        xlab = "year", main = "Posterior Samples", xaxt="none")
axis(1, seq(2019,2022,1))
dev.off()

sample_excess <- cbind(t = (samps_g_pred_all$t), sample_excess)
pdf(file = paste0(figure_path, "heart_excess_path.pdf"), height = 5, width = 5)
matplot(y = (sample_excess[,2:31]), x = sample_excess[,1], type = 'l', 
        add = F, lty = 'dashed', col = 'pink', ylab = "Heart Attack Excess Mortality",
        xlab = "year", main = "Posterior Samples", xaxt="none")
axis(1, seq(2019,2022,1))
dev.off()
