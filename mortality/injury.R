...
xInjury = x[grepl("Accidents", x$Cause) & x$province == "Ontario", ]
xPreCovid = xInjury[xInjury$date < as.Date("2020/03/01"),]
xPostCovid = xInjury[xInjury$date >= as.Date("2020/03/01"),]

# xPreCovid = xInjury # not worry about prediction for now
...
B1_full_exact <- rbind(0,Compute_design_Aug_sGP(t[-1])[1:(nrow(xInjury)-1), ])
...
plot(mean~t, data = samps_g_pred, type = 'l', ylim = c(4,5.5))
lines(upper~t, data = samps_g_pred, lty = 'dashed')
lines(lower~t, data = samps_g_pred, lty = 'dashed')
points(log(xInjury$VALUE)~I(sort(t) + 2010))
...
plot(mean~t, data = samps_g_pred, type = 'l', ylim = c(50,200))
lines(upper~t, data = samps_g_pred, lty = 'dashed')
lines(lower~t, data = samps_g_pred, lty = 'dashed')
points((xInjury$VALUE)~I(sort(t) + 2010))
...
pdf(file = paste0(figure_path, "Injury_log.pdf"), height = 5, width = 5)
plot((xInjury$VALUE)~I(sort(t) + 2010), cex = 0.5, lwd = 0.3, log = "y", type = "p",
     col = rgb(0,0,0,alpha = 0.3), xlab = "year", ylab = "Injury Death", ylim = c(50, 200))
...
pdf(file = paste0(figure_path, "Injury.pdf"), height = 5, width = 5)
plot((xInjury$VALUE)~I(sort(t) + 2010), cex = 0.5, lwd = 0.3, type = "p",
     col = rgb(0,0,0,alpha = 0.3), xlab = "year", ylab = "Injury Death", ylim = c(50, 200))
lines(mean~t, data = samps_g_pred, col = "blue", lwd = 1.3)
lines(upper~t, data = samps_g_pred, lty = 'dashed', col = "red", lwd = 1.3)
lines(lower~t, data = samps_g_pred, lty = 'dashed', col = "red", lwd = 1.3)
...
sample_excess_log <- log(xInjury$VALUE[-(1:nrow(xPreCovid))]) - log(samps_g_pred_all_obs[,1:3000])
sample_excess_log_total <- apply(na.omit(sample_excess_log), 2, sum)
pdf(file = paste0(figure_path, "Injury_excess_log.pdf"), height = 5, width = 5)
hist((sample_excess_log_total), xlab = "Injury Total Excess Mortality (Log)", main = "Posterior Samples", breaks = 30)
dev.off()
sample_excess <- (xInjury$VALUE[-(1:nrow(xPreCovid))]) - (samps_g_pred_all_obs[,1:3000])
sample_excess_total <- apply(na.omit(sample_excess), 2, sum)
pdf(file = paste0(figure_path, "Injury_excess.pdf"), height = 5, width = 5)
hist((sample_excess_total), xlab = "Injury Total Excess Mortality", main = "Posterior Samples", breaks = 30)
dev.off()


sample_excess_log <- cbind(t = (samps_g_pred_all$t), sample_excess_log)
pdf(file = paste0(figure_path, "Injury_log_excess_path.pdf"), height = 5, width = 5)
matplot(y = (sample_excess_log[,2:31]), x = sample_excess_log[,1], type = 'l',
        add = F, lty = 'dashed', col = 'pink', ylab = "Injury Excess Mortality (Log)",
        xlab = "year", main = "Posterior Samples", xaxt="none")
axis(1, seq(2019,2022,1))
dev.off()

sample_excess <- cbind(t = (samps_g_pred_all$t), sample_excess)
pdf(file = paste0(figure_path, "Injury_excess_path.pdf"), height = 5, width = 5)
matplot(y = (sample_excess[,2:31]), x = sample_excess[,1], type = 'l', 
        add = F, lty = 'dashed', col = 'pink', ylab = "Injury Excess Mortality",
        xlab = "year", main = "Posterior Samples", xaxt="none")
axis(1, seq(2019,2022,1))
dev.off()
